...
const Alert: React.FC<AlertProps> = ({ type, message, onClose }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className={`${bgColor[type]} border-l-4 p-4 mb-4 rounded-md`}
      role="alert"
    >
      <div className="flex items-center">
        ...
        {onClose && (
          <div className="ml-auto pl-3">
            <button
              onClick={onClose}
              className={`-mx-1.5 -my-1.5 rounded-md p-1.5 inline-flex ${iconColor[type]} hover:bg-${type === 'success' ? 'green' : type === 'error' ? 'red' : type === 'info' ? 'blue' : 'yellow'}-100 focus:outline-none`}
            >
