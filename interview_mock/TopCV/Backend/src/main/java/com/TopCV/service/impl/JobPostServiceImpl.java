...
public class JobPostServiceImpl implements JobPostService {
    ...
    @Override
    public PageResponse<JobPostResponse> getAllJobPosts(int page, int size) {
        Pageable pageable = PageRequest.of(page - 1, size, Sort.by("createdAt").descending());
        
        // Only get ACTIVE job posts from ACTIVE companies
        Page<JobPost> pageData = jobPostRepository.findByStatusAndCompanyActiveTrue(
                JobPostStatus.ACTIVE, pageable);

        List<JobPostResponse> jobResponses = pageData.getContent().stream()
                .map(jobPost -> {
                    JobPostResponse response = jobPostMapper.toResponse(jobPost);
                    
                    // Add favorite and apply status if user is authenticated
                    try {
                        String email = SecurityContextHolder.getContext().getAuthentication().getName();
                        if (email != null && !email.equals("anonymousUser")) {
                            User user = userRepository.findByEmail(email).orElse(null);
                            if (user != null) {
                                response.setIsFavorite(user.getFavoriteJobs().contains(jobPost));
                                ...
                                response.setCanApply(!hasApplied);
                            }
                        }
                    } catch (Exception e) {
                        // Anonymous user or authentication error
    ...
    @Override
    public PageResponse<JobPostResponse> searchJobPosts(String keyword, String location,
                                                       Integer jobTypeId, Integer jobLevelId,
                                                       List<Integer> skillIds, int page, int size) {
        Pageable pageable = PageRequest.of(page - 1, size, Sort.by("createdAt").descending());
        
        // Use repository method with search criteria
        Page<JobPost> pageData = jobPostRepository.searchJobPosts(
                keyword, location, jobTypeId, jobLevelId, skillIds, 
                JobPostStatus.ACTIVE, pageable);
