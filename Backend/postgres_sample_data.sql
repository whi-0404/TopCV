...
-- Insert Job Posts (với deadline tương lai)
...
(6, 'Internship Java Developer',
   ...
   '5-8 triệu', 'Hà Nội', '2025-06-30', '0-1 năm', 'ACTIVE', 1, 5, 1, 45, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
SELECT setval('company_categories_id_seq', (SELECT COALESCE(MAX(id), 1) FROM company_categories));
SELECT setval('companies_id_seq', (SELECT COALESCE(MAX(id), 1) FROM companies));
SELECT setval('job_types_id_seq', (SELECT COALESCE(MAX(id), 1) FROM job_types));
SELECT setval('job_levels_id_seq', (SELECT COALESCE(MAX(id), 1) FROM job_levels));
SELECT setval('skills_id_seq', (SELECT COALESCE(MAX(id), 1) FROM skills));
SELECT setval('job_posts_id_seq', (SELECT COALESCE(MAX(id), 1) FROM job_posts));
SELECT setval('resumes_id_seq', (SELECT COALESCE(MAX(id), 1) FROM resumes));
